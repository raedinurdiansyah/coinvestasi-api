 ...
 * @method bool          save(string $id, $data, int $lifetime = 0)
...
class Cache
{
    ...
    const DATABASE     = '\\NusaRaya\\Api\\CacheDriver\\DatabaseCache';
    ...
    protected static $drivers = [
        ...
        'apcu' => self::APCU,
        'file' => self::FILE,
        'wincache' => self::WIN_CACHE,
        'xcache' => self::X_CACHE,
        'x-cache' => self::X_CACHE,
        'zend' => self::ZEND,
        'array' => self::ARRAYS,
        'arrays' => self::ARRAYS,
        'chain' => self::CHAIN,
    ...
    /**
     * @var CacheProvider|MemcachedCache|MemcacheCache|RedisCache|CouchbaseCache|RiakCache|DatabaseCache
     */
    protected $resource;
    ...
    protected function sanitizeTest(array $config)
    {
        ...
        if (is_string($driver) && isset(self::$drivers[strtolower($driver)])) {
            $this->config['driver'] = self::$drivers[strtolower($driver)];
        }
        ...
        switch ($this->config['driver']) {
            ...
            case self::PHP:
                if (! $this->resource) {
                    ...
                    $directory = !$directory && isset($config['path']) ? $config['path'] : null;
            ...
            default:
                if (is_string($this->config['driver'])
                    && class_exists($this->config['driver'])
                    && is_subclass_of($this->config['driver'], CacheProvider::class)
                ) {
                    $this->resource = new $this->config['driver'];
                }
        }
